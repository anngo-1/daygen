...
public:
        ...
        , beta(beta)
    {
        ...
        if (alpha + beta >= 1) {
            std::cerr << "Warning: GARCH parameters alpha + beta >= 1, may lead to non-stationarity" << std::endl; // Warning, not exception
        }
    }
    ...
    void update(T r) {
        ...
        if (sigma2 <= 0) {
            ...
            sigma2 = std::numeric_limits<T>::epsilon(); // Clamp to a small positive value
        }
...
std::vector<T> calculateEMA(const std::vector<T>& prices, int period) {
    ...
    ema[0] = prices[0]; // initialize first EMA value
